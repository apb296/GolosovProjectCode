function [ grad, iflag] = BelObjectiveUncondGradNAG2(n,x,iflag)
...
%   Detailed explanation goes here
    ...
    [l1 l1grad] = computeL(c1_1,c1_2,c2_1,c2_2,grad_c2_2,theta_1,g);
    
    [btildprime grad_btildprime] = computeBtildeprime(c1_1,c1_2,c2_1,c2_2,grad_c2_2,l1,l1grad,...
  ...
  %check typo
    %  grad1(1) = P(s_,1)*(alpha(1)*c1_1^(-sigma)+beta*sigma*c1_1^(sigma-1)*dV(2));
      grad1(1) = P(s_,1)*(alpha(1)*c1_1^(-sigma)+beta*sigma*c1_1^(sigma-1)*dV(2)*c2_1^(-sigma));
...
%    grad1(3) = P(s_,1)*(alpha(2)*c2_1^(-sigma)-beta*sigma*c2_1^(-sigma-1)*(btildprime(1)+dV(2)));
        grad1(3) = P(s_,1)*(alpha(2)*c2_1^(-sigma)-beta*sigma*c2_1^(-sigma-1)*(btildprime(1)*dV(1)+dV(2)*c1_1^(sigma)));
    ...
    grad2(2) = P(s_,2)*(alpha(1)*c1_2^(-sigma)+beta*sigma*c1_2^(sigma-1)*dV(2)*c2_2^(-sigma));

     grad2(3) = 0;
         %check typo

    %d_c2_2 = P(s_,2)*(alpha(2)*c2_2^(-sigma)-beta*sigma*c2_2^(-sigma-1)*(dV(2)+btildprime(2)));
    d_c2_2 = P(s_,2)*(alpha(2)*c2_2^(-sigma)-beta*sigma*c2_2^(-sigma-1)*(dV(2)*c1_2^(sigma)+dV(1)*btildprime(2)));
    
    grad2 = grad2 + d_c2_2*grad_c2_2;
    grad2 = grad2 + P(s_,2)*( grad_btildprime(:,2)*c2_2^(-sigma)*beta*dV(1)...
        -alpha(1)*l1(2)^gamma*l1grad(:,2));
...
function [l1 l1grad] = computeL(c1_1,c1_2,c2_1,c2_2,grad_c2_2,theta_1,g)
    
    l1(1) = (c1_1+c2_1+g(1))/theta_1;
    l1grad = zeros(3,2);
    %compute gradient
    l1grad(1,1) = 1/theta_1; %wrt c1_1
    l1grad(3,1) = 1/theta_1; %wrt c2_1
    
    l1(2) = (c1_2+c2_2+g(2))/theta_1;
    %Compute gradient
    l1grad(2,2) = 1/theta_1;
    l1grad(:,2) = l1grad(:,2)+grad_c2_2/theta_1;

end

function [btildprime grad_btildprime] = computeBtildeprime(c1_1,c1_2,c2_1,c2_2,grad_c2_2,l1,l1grad,...
   u2btild,s_,sigma,gamma,beta,P)
    ...
    grad_btildprime(3,1) = sigma*u2btild*P(s_,1)*c2_1^(-sigma-1)/(beta*Eu2^2)-1;
    ...
    grad_btildprime(:,1) = grad_btildprime(:,1) + d_c2_2*grad_c2_2+d_l1_1*l1grad(:,1);
    ...
    d_c2_2 = sigma*u2btild*P(s_,2)*c2_2^(-sigma-1)/(beta*Eu2^2)-1;
    d_l1_2 = -(1+gamma)*l1(2)^gamma*c1_2^sigma;

    grad_btildprime(:,2) = grad_btildprime(:,2) + d_c2_2*grad_c2_2+d_l1_2*l1grad(:,2);
