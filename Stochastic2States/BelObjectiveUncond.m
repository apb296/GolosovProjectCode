function [ Vobj grad ] = BelObjectiveUncond( c1_1,c1_2,c2_1,V,Vcoef,u1,u2,btild,s_ ,Par)
...
%   Detailed explanation goes here
    ...
    [c2_2 grad_c2_2] = computeC2_2(c1_1,c1_2,c2_1,u1,u2,s_,P,sigma);
   ...
   u2,btild,s_,sigma,gamma,beta,P);
    
    %compute objective
    grad1 = zeros(3,1);
    X = [btildprime(1),c1_1^(-sigma),c2_1^(-sigma)];%state next period
    Vobj = P(s_,1)*(alpha(1)*U(c1_1,l1(1),sigma,gamma)+alpha(2)*U(c2_1,l2(1),sigma,gamma)...
            +beta*funeval(Vcoef{1},V{1},X));
    dV = funeval(Vcoef{1},V{1},X,eye(3));
    grad1(1) = P(s_,1)*(alpha(1)*c1_1^(-sigma)-beta*sigma*c1_1^(-sigma-1)*dV(2));
    grad1(2) = 0;
    grad1(3) = P(s_,1)*(alpha(2)*c2_1^(-sigma)-beta*sigma*c2_1^(-sigma-1)*dV(3));
    
    grad1 = grad1+P(s_,1)*( grad_btildprime(:,1)*beta*dV(1) - alpha(1)*l1(1)^gamma*l1grad(:,1)...
        -alpha(2)*l2(1)^gamma*l2grad(:,1));
    
    grad2 = zeros(3,1);
    X = [btildprime(2),c1_2^(-sigma),c2_2^(-sigma)];%state next period
    Vobj = Vobj + P(s_,2)*(alpha(1)*U(c1_2,l1(2),sigma,gamma)+alpha(2)*U(c2_2,l2(2),sigma,gamma)...
            +beta*funeval(Vcoef{2},V{2},X));
    dV = funeval(Vcoef{2},V{2},X,eye(3));
    grad2(1) = 0;
    grad2(2) = P(s_,2)*(alpha(1)*c1_2^(-sigma)-beta*sigma*c1_2^(-sigma-1)*dV(2));
    grad2(3) = 0;
    
    d_c2_2 = P(s_,2)*(alpha(2)*c2_2^(-sigma)-beta*sigma*c2_2^(-sigma-1)*dV(3));
    
    grad2 = grad2 + d_c2_2*grad_c2_2;
    grad2 = grad2 + P(s_,2)*( grad_btildprime(:,2)*beta*dV(1)-alpha(1)*l1(2)^gamma*l1grad(:,2)...
...
function [ c2_2 grad ] = computeC2_2(c1_1,c1_2,c2_1,u1,u2,s_,P,sigma)

    %Compute c2_2 from formula
    frac = (u2*P(s_,1)*c1_1^(-sigma)+u2*P(s_,2)*c1_2^(-sigma)-u1*P(s_,1)*c2_1^(-sigma))...
        /( u1*P(s_,2) );
    c2_2 = frac^(-1/sigma);
    grad=zeros(3,1);
    %compute the gradients for c1_1,c1_2,c2_1
    grad(1) = c1_1^(-sigma-1)*frac^(-1/sigma-1)*u2*P(s_,1)/(u1*P(s_,2));
    grad(2) = c1_2^(-sigma-1)*frac^(-1/sigma-1)*u2/(u1);
...
function [btildprime grad_btildprime] = computeBtildeprime(c1_1,c1_2,c2_1,c2_2,grad_c2_2,l1,l2,l1grad,l2grad,...
   u2,btild,s_,sigma,gamma,beta,P)
    ...
    btildprime(1) = u2*btild/(beta*Eu2)...
    ...
    grad_btildprime(3,1) =sigma*u2*btild*P(s_,1)*c2_1^(-sigma-1)/(beta*Eu2^2)...
        -1+sigma*l2(1)^(1+gamma)*c2_1^(sigma-1);

    %figure out their affects through c2_2, l1_1,l2_1
    d_c2_2 = sigma*u2*btild*P(s_,2)*c2_2^(-sigma-1)/(beta*Eu2^2);
    ...
    btildprime(2) = u2*btild/(beta*Eu2)...
    ...
    grad_btildprime(3,2) = sigma*u2*btild*P(s_,1)*c2_1^(-sigma-1)/(beta*Eu2^2);
    %figure out their affects through c2_2, l1_2,l2_2
    d_c2_2 = sigma*u2*btild*P(s_,2)*c2_2^(-sigma-1)/(beta*Eu2^2)-1+sigma*l2(2)^(1+gamma)*c2_2^(sigma-1);
