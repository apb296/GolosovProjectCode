...
{
  double *a, *b, *x, *c, *y;
  double *cptr, *ccol, *xptr, *yptr, *cin, *cend, *cp, *cstart;
  double z, z2, dd, di, temp;
  unsigned long *cind, *n;
  unsigned long d, m, p, N, i, j, nj, xrows, csize;
  ...
  a=mxGetPr(prhs[2]);
  n=mxCalloc(d,sizeof(unsigned long));
  for (i=0;i<d;i++) n[i]=(unsigned long)a[i];
  ...
  cind=mxCalloc(d,sizeof(unsigned long));  
